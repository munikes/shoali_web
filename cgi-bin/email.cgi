...
my $emailConfPath = "../email/email.conf";
...
my $name = &veryclean($query->param('inputEmail'));
#my $email = &veryclean($query->param('email'));
my $email = 'info@shoali.org';
#my $recipient = &veryclean($query->param('recipient'));
my $recipient = &veryclean($query->param('inputEmail'));
#my $subject = &veryclean($query->param('subject'));
my $subject = '[Shoali] Shoali newsletter subscription.';
#newlines allowed
#my $content = &clean($query->param('content'));
my $content = 'Thank you for your interest. I hope to have more good news soon
\n Muchas gracias por su interés. Espero poderle dar buenas noticias pronto.';
my $bcc = 'info@shoali.org';

#Note: subject is not mandatory, but you can easily change that
if (($name eq "") || ($email eq "") || ($content eq "") || ($recipient eq ""))
{
	&error("Emmail Rejected",
		"Please fill out all fields provided. Back up to the " .
		"previous page to try again.$name $email $content $recipient");
}
